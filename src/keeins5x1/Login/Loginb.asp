...
<%
	...
	if not rs.eof then
		Session.Contents("GroupType@KEEINS5x1")		= rs("GroupType")
		Session.Contents("Owner@KEEINS5x1")			= ""

		if Len(CStr(rs("Owner"))) > 0 then
			if CStr(rs("Owner")) <> "-" then
				Session.Contents("Owner@KEEINS5x1")	= CStr(rs("Owner"))
			end if
		end if
		...
		Session.Contents("GroupType@KEEINS5x1") = -1 
