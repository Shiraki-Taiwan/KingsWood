...
<%
	...
	if not rs.eof then
		Session.Contents("GroupType@KEEINS7")		= rs("GroupType")
		Session.Contents("Owner@KEEINS7")			= ""

		if Len(CStr(rs("Owner"))) > 0 then
			if CStr(rs("Owner")) <> "-" then
				Session.Contents("Owner@KEEINS7")	= CStr(rs("Owner"))
			end if
		end if
		...
		Session.Contents("GroupType@KEEINS7") = -1 
