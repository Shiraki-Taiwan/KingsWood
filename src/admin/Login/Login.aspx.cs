	...
	protected void Page_Load(object sender, EventArgs e)
	{
		if (!IsPostBack)
		{
			if (!string.IsNullOrEmpty(Request.Form["ID"]))
			{
				...
				{
					Session["GroupType@ADMIN"] = Guid.NewGuid().ToString();
		...
		if (Session["GroupType@ADMIN"] == null)
			Response.Write(Session["GroupType@ADMIN"] == null);
		else
			Response.Write(Session["GroupType@ADMIN"].ToString());
	}
}