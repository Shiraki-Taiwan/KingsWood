...
<%
	...
	if not rs.eof then
		Session.Contents("GroupType@KAOINS7")		= rs("GroupType")
		Session.Contents("Owner@KAOINS7")			= ""

		if Len(CStr(rs("Owner"))) > 0 then
			if CStr(rs("Owner")) <> "-" then
				Session.Contents("Owner@KAOINS7")	= CStr(rs("Owner"))
			end if
		end if
		...
		Session.Contents("GroupType@KAOINS7") = -1 
