...
<%
	...
	if not rs.eof then
		Session.Contents("GroupType@" & szSessionKey)		= rs("GroupType")
		Session.Contents("Owner@" & szSessionKey)			= ""

		if Len(CStr(rs("Owner"))) > 0 then
			if CStr(rs("Owner")) <> "-" then
				Session.Contents("Owner@" & szSessionKey)	= CStr(rs("Owner"))
			end if
		end if
		...
		Session.Contents("GroupType@" & szSessionKey) = -1 
