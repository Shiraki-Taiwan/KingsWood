...
<%
	...
	if not rs.eof then
		Session.Contents("GroupType@KAOINS69")		= rs("GroupType")
		Session.Contents("Owner@KAOINS69")			= ""

		if Len(CStr(rs("Owner"))) > 0 then
			if CStr(rs("Owner")) <> "-" then
				Session.Contents("Owner@KAOINS69")	= CStr(rs("Owner"))
			end if
		end if
		...
		Session.Contents("GroupType@KAOINS69") = -1 
